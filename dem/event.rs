...
mod tests {
    ...
    fn event_initial_output_respected() {
        let c_cfg = CalibConfig { step_up: &2, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze , debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
    ...
    fn event_count_1_confirms_in_1_tick() {
        ...
        let c_cfg = CalibConfig { step_up: &1, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze , debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
        c.step(Status::PreFailed, true, 0.0);
        assert_eq!(c.debounce_counter(), i16::MAX);
    ...
    fn event_count_0_leaves_counter_unchanged() {
        ...
        let c2_cfg = CalibConfig { step_up: &0, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5 };
        let mut nv2_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
    ...
    fn event_prepassed_count_2_heals_in_ticks() {
        ...
        let c_cfg = CalibConfig { step_up: &0, step_down: &2, debounce_behavior: &DebounceBehavior::Freeze , debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
    ...
    fn event_holds_true_in_middle() {
        let c_cfg = CalibConfig { step_up: &1, step_down: &2, debounce_behavior: &DebounceBehavior::Freeze , debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
    ...
    fn event_not_complete_starts_true() {
        let c_cfg = CalibConfig { step_up: &0, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
    ...
    fn event_not_complete_stays_false_after_negative_threshold() {
        let c_cfg = CalibConfig { step_up: &2, step_down: &1, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        nv_config.uds_status.set_tf(true);
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
    ...
    fn event_freeze_holds_counter_and_tf() {
        let c_cfg = CalibConfig { step_up: &3, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze , debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
    ...
    fn event_reset_zeroes_counter_when_disabled() {
        let c_cfg = CalibConfig { step_up: &2, step_down: &0, debounce_behavior: &DebounceBehavior::Reset , debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
        c.step(Status::PreFailed, true, 0.0);
        assert_eq!(c.debounce_counter(), div_round(i16::MAX,2));
    ...
    #[test]
    fn aging_cycles_increments_on_stop_when_tfslc_but_not_tftoc() {
        let c_cfg = CalibConfig { step_up: &1, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
        // Fail to set tfslc
        ...
        // Stop: since tftoc is true, aging_cycles should not increment
        c.stop();
        assert_eq!(*c.nv_config.aging_cycles, 0u8);
    }

    #[test]
    fn aging_cycles_increments_on_stop_when_tfslc_true_and_tftoc_false() {
        let c_cfg = CalibConfig { step_up: &1, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
        c.nv_config.uds_status.set_cdtc(true);
        c.nv_config.uds_status.set_tnctoc(false);
        c.nv_config.uds_status.set_tfslc(true);
        assert!(c.status().cdtc());
        assert!(c.status().tfslc());
        assert!(!c.status().tftoc());
        assert!(!c.status().tnctoc());
        c.stop();
        assert_eq!(*c.nv_config.aging_cycles, 1u8);
    }

    #[test]
    fn aging_cycles_not_incremented_when_tftoc_true_on_stop() {
        let c_cfg = CalibConfig { step_up: &1, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
        // Fail to set both
        ...
        assert!(c.status().tfslc());
        assert!(c.status().tftoc());
        // Stop: tftoc true, so aging_cycles not incremented
        c.stop();
        assert_eq!(*c.nv_config.aging_cycles, 0u8);
        // But confirmation_cycles should increment
        assert_eq!(*c.nv_config.confirmation_cycles, 1u8);
    }

    #[test]
    fn confirmation_cycles_increments_on_stop_when_tftoc_true() {
        let c_cfg = CalibConfig { step_up: &1, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 0u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
        // Fail to set tftoc
        c.step(Status::PreFailed, true, 0.0);
        assert!(c.status().tftoc());
        // Stop: tftoc true, so increment confirmation_cycles
        c.stop();
        assert_eq!(*c.nv_config.confirmation_cycles, 1u8);
        // Stop again: increment again
        c.stop();
        assert_eq!(*c.nv_config.confirmation_cycles, 2u8);
    }

    #[test]
    fn confirmation_cycles_resets_on_clear() {
        let c_cfg = CalibConfig { step_up: &1, step_down: &0, debounce_behavior: &DebounceBehavior::Freeze, debounce_type: &DebounceType::CounterBased, confirmation_threshold: &3, aging_threshold: &5};
        let mut nv_config = NvmConfig { uds_status: &mut UdsStatusByte::new(0), occurence_cntr: &mut 0u8 , aging_cycles: &mut 0u8, confirmation_cycles: &mut 5u8};
        let mut c: Event<'_, '_> = Event::new(&mut nv_config, &c_cfg);
        // Initially 5
        assert_eq!(*c.nv_config.confirmation_cycles, 5u8);
        // Clear: should reset to 0
        c.clear();
        assert_eq!(*c.nv_config.confirmation_cycles, 0u8);
    }
